...
public function generateLaporanHarian($proses,$data, $objPHPExcel, $rendererName, $rendererLibraryPath, $objWriter, $objSheet)
    {
	...
	function executiveSummary($index,$nomorKK,$objPHPExcel, $objWriter, $objSheet)
        {
			...
			foreach($dataEmboss as $row){
				if($row->JAM_PROSES<0){
					$emboss[0] = $emboss[0]+(86400+round(str_replace(",",".",$row->JAM_PROSES), 0, PHP_ROUND_HALF_UP));
				}else{
				$emboss[0] = $emboss[0]+(round(str_replace(",",".",$row->JAM_PROSES), 0, PHP_ROUND_HALF_UP));	
				}

				if($row->A<0){
					$emboss[1] = $emboss[1]+(86400+round(str_replace(",",".",$row->A), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[1] = $emboss[1]+(round(str_replace(",",".",$row->A), 0, PHP_ROUND_HALF_UP));	
				}

				if($row->B<0){
					$emboss[2] = $emboss[2]+(86400+round(str_replace(",",".",$row->B), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[2] = $emboss[2]+(round(str_replace(",",".",$row->B), 0, PHP_ROUND_HALF_UP));	
				}
				
				if($row->C<0){
					$emboss[3] = $emboss[3]+(86400+round(str_replace(",",".",$row->C), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[3] = $emboss[3]+(round(str_replace(",",".",$row->C), 0, PHP_ROUND_HALF_UP));	
				}
				if($row->D<0){
					$emboss[4] = $emboss[4]+(86400+round(str_replace(",",".",$row->D), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[4] = $emboss[4]+(round(str_replace(",",".",$row->D), 0, PHP_ROUND_HALF_UP));	
				}
				if($row->E<0){
					$emboss[5] = $emboss[5]+(86400+round(str_replace(",",".",$row->E), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[5] = $emboss[5]+(round(str_replace(",",".",$row->E), 0, PHP_ROUND_HALF_UP));	
				}
				if($row->F<0){
					$emboss[6] = $emboss[6]+(86400+round(str_replace(",",".",$row->F), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[6] = $emboss[6]+(round(str_replace(",",".",$row->F), 0, PHP_ROUND_HALF_UP));	
				}
				if($row->G<0){
					$emboss[7] = $emboss[7]+(86400+round(str_replace(",",".",$row->G), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[7] = $emboss[7]+(round(str_replace(",",".",$row->G), 0, PHP_ROUND_HALF_UP));	
				}
				
				if($row->H<0){
					$emboss[8] = $emboss[8]+(86400+round(str_replace(",",".",$row->H), 0, PHP_ROUND_HALF_UP));
				}else{
					$emboss[8] = $emboss[8]+(round(str_replace(",",".",$row->H), 0, PHP_ROUND_HALF_UP));	
				}
			}
			// exit();
...
function laporanBulanan(){
		...
		if($session!=""){
			...
			if($data["status"]=="ADMINPROD"){
				$this->load->view('AdminProduksi/v_header',$data);
				$this->load->view('AdminProduksi/v_sidebar',$data);
				$this->load->view('AdminProduksi/v_laporan_bulanan',$data);
				$this->load->view('AdminProduksi/v_footer',$data);
			}else{
				?>
				<script type="text/javascript" language="javascript">
	...
	public function generateLaporanBulanan(){
			...
			$filename = "TEST LAPORAN PRODUKSI PER BULAN";
			// We'll be outputting an excel file
			header('Content-type: application/vnd.ms-excel');
			// It will be called file.xls
			header('Content-Disposition: attachment; filename="'.$filename.'.xlsx"');
