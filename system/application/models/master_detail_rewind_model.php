...
class Master_detail_rewind_model extends Model
{
	...
	function findByMonth($bulan,$tahun,$mesin){
    	...
    	$this->oracle_db->where('MESIN_REWIND',$mesin);
		$t=$this->oracle_db->get();
		return $t->result();
	}
	...
	function updateData($noUrut, $data){
		$this->oracle_db=$this->load->database('oracle',true);
		$this->oracle_db->trans_begin();
		$this->oracle_db->where('NO_URUT_REWIND',$noUrut);
		$success = $this->oracle_db->update('TBL_DETAIL_REWIND', $data);
		$this->oracle_db->trans_commit();
		$this->oracle_db->trans_complete();
			if(!$success){
	...
	function updateDataAndMutasi($noUrut, $data, $dataMutasi){
		...
		if($success){
			...
			$success = $this->oracle_db->update('TBL_MUTASI_DEMET', $dataMutasi);
		}else{
			$success = false;
			$errNo   = $this->oracle_db->_error_number();
			$errMess = $this->oracle_db->_error_message();
