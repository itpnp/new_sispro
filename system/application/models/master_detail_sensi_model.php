...
function findByRollBeforeMutation($kodeRoll){
	...
	function findById($id){
		$this->oracle_db=$this->load->database('oracle',true);
		$this->oracle_db->where("NO_URUT_SENSI", $id);
		$t=$this->oracle_db->get('TBL_DETAIL_SENSI');
		return $t->row();
	}
	...
	function updateDataAndMutasi($noUrut, $data, $dataMutasi){
		$this->oracle_db=$this->load->database('oracle',true);
		$this->oracle_db->trans_begin();
		$this->oracle_db->where('NO_URUT_SENSI',$noUrut);
		$success = $this->oracle_db->update('TBL_DETAIL_SENSI', $data);
		if($success){
			$this->oracle_db->where('NO_MUTASI',$dataMutasi['NO_MUTASI']);
			$success = $this->oracle_db->update('TBL_MUTASI_REWIND', $dataMutasi);
		}else{
			$success = false;
			$errNo   = $this->oracle_db->_error_number();
			$errMess = $this->oracle_db->_error_message();
